...
class CoquiTTSManager:
        ...
        try:
            ...
            if cached_audio is not None:
                ...
                with self._lock:
                    ...
                    speaker_wav_path = self._get_speaker_wav_path()

                    if not speaker_wav_path:
                        # Use enhanced fallback if enabled, otherwise basic fallback
                        if self.config.tts_fallback_enabled:
                            logger.info("No voice profile available, using enhanced fallback TTS")
                            self._speak_with_default_voice(text)
                        else:
                            logger.info("No voice profile available, using basic fallback TTS")
    ...
    def _speak_with_default_voice(self, text: str) -> None:
        """
        Speak using a built-in default voice.
        Since XTTS requires a speaker reference, we'll use a high-quality fallback.
        """
        try:
            logger.info("Using enhanced default TTS voice")

            # Try to use a better TTS engine than basic pyttsx3
            # This provides more consistent results than XTTS without voice cloning
            self._enhanced_fallback_speak(text)

        except Exception as e:
            logger.warning(f"Enhanced default TTS failed: {e}, using basic fallback")
            self._fallback_speak(text)

    def _enhanced_fallback_speak(self, text: str) -> None:
        """
        Enhanced fallback TTS with better voice quality and consistency.
        """
        try:
            import pyttsx3

            logger.info("Using enhanced pyttsx3 TTS with optimized settings")
            engine = pyttsx3.init()

            # Get available voices and select the best one
            voices = engine.getProperty('voices')
            if voices:
                # Use configurable voice preferences
                preferred_voices = self.config.tts_fallback_voices
                selected_voice = None
                ...
                if selected_voice:
                    ...
                    logger.debug(f"Selected voice: {selected_voice.name}")

            # Optimize settings for consistency (using configurable rate cap)
            engine.setProperty('rate', min(self.config.tts_rate, self.config.tts_fallback_rate_cap))
            ...
            logger.debug("Enhanced TTS completed")
    ...
    def _fallback_speak(self, text: str) -> None:
        """
        Fallback TTS method when neural TTS is not available.
        Uses system TTS as a last resort.
        """
        try:
            import pyttsx3
