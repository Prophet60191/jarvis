#!/usr/bin/env python3
"""
EXACT WORKING JARVIS STARTUP - From Git Commit aa88a13

This is the EXACT startup script that was working in the git repository.
Uses the proper directory structure and initialization sequence.
"""
...
def start_jarvis(mode="voice", debug=False):
    ...
    try:
        if mode == "voice":
            # Start full voice assistant - EXACT command from working git
            os.chdir("jarvis")
            if debug:
...
def main():
    """Main startup function - EXACT from working git."""
    parser = argparse.ArgumentParser(
        description="Start Jarvis Voice Assistant",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python start_jarvis_exact.py                    # Start voice assistant
  python start_jarvis_exact.py --mode ui          # Start web UI only
  python start_jarvis_exact.py --mode test        # Run test mode
  python start_jarvis_exact.py --debug            # Start with debug logging
  python start_jarvis_exact.py --skip-checks      # Skip system checks
        """
    )
